...
**就労継続支援B型**の1事業所が、利用者管理・個別支援計画・支援記録・出欠/実績記録票・
期限アラートを、**事業所のPC1台とLANだけ**で回すためのオープンソース業務支援アプリです。
月額料金・クラウド・外部送信はありません。

---

## これは何か（30秒で判定）

利用者数 約35名 / 支援スタッフ 約6名 を想定した、**小規模・単一事業所向け**の業務支援アプリです。
利用者情報・個別支援計画・進捗記録・日々の支援記録・出欠/実績記録票（月次・印刷対応）・
モニタリング期限/受給者証期限のアラートを一つの画面に集約します。

名前の「歩み」は設計思想を表しています。計画の段階遷移、目標の進捗、日々の記録、出欠——
すべてを **追記専用のログ** として残し、最新行から「現在の状態」を導出します。過去の記録は
上書きせず、訂正も新しい行として残すため、福祉記録としての履歴が失われません。

### こんな事業所向け

- 月額料金やクラウドへの囲い込みを避け、**データを自分の手元（事業所のPC）に置きたい**
- インターネットに依存せず、**LAN内だけで完結**させたい
- 必要なら、自分（または委託先）で**改造できる**ものがよい
- Elixir/OTP の知識がなくても、**ビルド済みバイナリで導入**したい

### 向いていない場合（正直に）

- **国保連への請求まで1本化したい** → ポチパスや knowbe 等の商用ソフトが適します。
  歩みは**記録・管理の層に特化**しており、請求機能は持ちません。実績データは外部の
  請求ソフトへ渡す前提です。
- クラウドで複数事業所を横断管理したい → 本アプリは単一ホスト・LAN 構成専用です。

> 設計の詳細・ドメインの考え方・実装順は [`CLAUDE.md`](CLAUDE.md) を参照してください。

---

## まず動かす（Elixir 不要・ビルド済みバイナリ）

Elixir や Erlang のインストールは不要です。事業所の PC 1 台で動きます。

1. [GitHub Releases](https://github.com/SilentMalachite/Ayumi/releases) から、お使いの OS の
   アーカイブをダウンロードして展開します。
   - Windows (x86_64): `ayumi-vX.Y.Z-windows-x86_64.zip`
   - macOS (Apple Silicon): `ayumi-vX.Y.Z-macos-arm64.tar.gz`
2. 起動スクリプトのテンプレートをコピーし、`SECRET_KEY_BASE`（64文字以上のランダム文字列）を設定します。
   - Windows: `copy start.bat.example start.bat`
   - macOS: `cp start.sh.example start.sh`
3. 職員アカウントを1つ作成します。
   - Windows: `bin\ayumi.bat eval "Ayumi.Release.create_user()"`
   - macOS: `bin/ayumi eval "Ayumi.Release.create_user()"`
4. 起動します（Windows: `start.bat` / macOS: `./start.sh`）。
   ブラウザで [`http://localhost:4000`](http://localhost:4000) を開きます。

LAN の他の端末からは `http://<このPCのIPアドレス>:4000` でアクセスできます。
詳しい手順・ソースからのビルド・バージョンアップは [本番デプロイ](#本番デプロイ) を参照してください。

> **データの置き場所について（重要）:** DB ファイル（`data/` フォルダ）は、ネットワーク共有
> （SMB/NFS）やクラウド同期フォルダ（OneDrive / iCloud / Dropbox）に置かないでください。
> 同時アクセスで破損します。ホストのローカルディスクに置きます。
...
- **支援記録（support_record）**: 利用者ごとの日々の支援内容を追記専用ログとして記録。
  カテゴリ（作業・生活・健康・面談・その他）で分類し、利用者別・支援日の範囲でフィルタ可能。
  「支援日」（支援した日。既定は今日、未来日は不可）と「記録日時」（入力した瞬間）を分けて
  持つので、あとからまとめて入力しても、いつの支援かが正しく残ります。
  `/support_records` で全体一覧・新規作成。

### 出欠・実績記録票

- **出欠（attendance_record）**: 利用者ごとの日々の出欠・サービス提供内容を追記専用ログ
  として記録。提供形態（`provision_type`）は `通所 / 施設外就労 / 施設外支援 / 欠席 /
  欠席時対応`（`Ayumi.Plans.ProvisionType`）から選択し、送迎（pickup / dropoff）・
  提供時間（start_time / end_time）・所見を保存します。訂正も新しい行として追記し、
  同じ日付については最新行を採用します。
- **月次グリッド入力**: `/service_users/:id/attendance` で、1利用者・1か月分を1日1行の
  グリッドで入力（`AttendanceLive.Index`）。年月の前後ナビゲーション、開始時刻 ≤ 終了時刻の
  バリデーション、保存時の append（更新はしない）を行います。
- **実績記録票の印刷ビュー**: `/service_users/:id/attendance/sheet` で A4 印刷向け
  レイアウト（`AttendanceLive.Sheet`、`@page { size: A4 }`、ツールバーは `print:hidden`）。
  事業所名・事業所番号は `:ayumi, :facility` 設定（`config/config.exs` にひな型コメント、
  未設定なら空欄）から読み出し、合計（利用日数・施設外日数・送迎回数・欠席時対応回数）は
  画面上で再計算せず `build_attendance_sheet/3` の結果をそのまま表示します。
- 利用者詳細ページから「出欠・実績記録票」リンクで遷移できます。記録・閲覧は全認証
  スタッフが可能（管理者専用ではありません）。
...
- 利用者詳細ページ（`/service_users/:id`）に、基本情報に加えて
  ...
  全体像を把握できます。出欠・実績記録票への導線リンクも配置しています。
...
- **DB バックアップ**: サービス管理責任者専用。Web 画面（`/admin/backup`）と
  ...
  保存先パス・サイズ・保存時刻（日本時間）を表示します。ビルド済みリリースでも
  Web 画面から実行できます。
- **CSV 出力**: 全職員が `/exports` から、出欠・実績記録／支援記録／目標進捗の履歴／計画段階の
  履歴を、週（月曜始まり）／月／年度／暦年の単位で CSV として保存できます。Excel でそのまま
  開ける UTF-8（BOM 付き）です。出欠は利用者×利用日ごとの最新行（訂正後の内容）を、各ログは
  記録日時が期間内の行を古い順に出力します。日時と期間の境界は日本時間で、退所者の記録も
  含みます。利用者台帳（期間なし・全員の現在の登録内容）も出力できます。Excel は数字だけの
  セルの先頭ゼロ（受給者証番号など）を表示上落とすことがあります。CSV ファイル自体には
  正しく入っているので、必要なら Excel の「データの取り込み」で列を文字列に指定してください。CSV には個人情報が含まれる
  ため、保存先と持ち出しに注意してください。
- **CSV 取込**: サービス管理責任者専用。`/admin/import` で、Excel で作成・編集した出欠・実績
  記録の CSV を取り込めます。「CSV 出力」で保存したファイルはそのまま取り込めるので、
  出力 → Excel で編集 → 取込、という月単位の一括入力ができます。
  - 取込前に「追加／訂正／変更なし／エラー」を確認でき、1 行でもエラーがあれば何も取り込みません。
  - 追記のみ: すでに記録のある利用日は訂正として追記され、元の記録は履歴に残ります。同じ内容の
    行は取り込まれません。CSV から行を消しても記録は削除されません。
  - Excel で保存するときは、ファイルの種類を **「CSV UTF-8（コンマ区切り）」** にしてください
    （通常の「CSV」は Shift_JIS のため取り込めません）。見出しの行は変えないでください。
    利用日は `2026-09-01` か `2026/9/1` の形で。セルの表示形式を「9月1日」にすると年が失われ、
    エラーになります。
  - 利用者は `利用者ID` で特定します。空欄なら `氏名` が一意のときだけ特定できます。
  - **支援記録**も取り込めます（Excel にあった過去の記録の移行向け）。必要な列は `利用者ID`・`氏名`・
    `支援日`・`区分`・`内容` です。同じ内容の記録が既にある行は取り込まず、それ以外は新しい記録として
    追加します。出力した CSV の内容を書き換えて取り込んでも元の記録は変わらず、新しい記録が増える
    点に注意してください（確認画面で「注意」として表示します）。未来の支援日は取り込めません。
    退所した利用者の過去の記録は、取込に限って登録できます（画面からの入力はできません）。
    記録者は取り込んだ人、記録日時は取り込んだ時刻になります。
  - **利用者台帳**も取り込めます（初期導入時の名簿の一括登録向け）。新しい利用者だけを追加し、
    登録済みの利用者（`利用者ID`・`受給者証番号`・`氏名`＋`生年月日` のいずれかが一致）は変更せず
    スキップします。新しい利用者は `利用者ID` を空欄にしてください。障害者手帳の列は取り込み
    ません。受給者証番号・電話番号・郵便番号の列は、Excel で「文字列」にしておくと先頭の 0 が
    消えません（消えていそうな受給者証番号は、取込前の確認画面で「注意」として表示します）。
  - 上限は 2 MB・5,000 行です。年単位のデータは月ごとに分けて取り込んでください。
...
ビルド済みバイナリを使う場合は不要です（[まず動かす](#まず動かすelixir-不要ビルド済みバイナリ)）。
**ソースからビルドする場合**のみ、以下が必要です。
...
中心モデルは、2つの「本体」（めったに編集しない）と4つの「追記専用ログ」で構成します。
...
- `support_record` — 利用者ごとの日々の支援記録を1行ずつ記録する追記専用ログ
  （カテゴリ・内容・記録者・日時）。
- `attendance_record` — 利用者ごとの日々の出欠・サービス提供を1行ずつ記録する追記専用
  ログ（提供形態・送迎・提供時間・所見・記録者・日時）。月次の実績記録票は
  `Ayumi.Plans.AttendanceSheet` として `build_attendance_sheet/3` で導出（保存しない）。
...
サービス提供形態（`attendance_record.provision_type`、`Ayumi.Plans.ProvisionType`）:

`commute`（通所） / `offsite_work`（施設外就労） / `offsite_support`（施設外支援） /
`absence`（欠席） / `absence_support`（欠席時対応）

`ProvisionType.billable/0` は利用日数の算定対象（通所・施設外就労・施設外支援）、
`ProvisionType.offsite/0` は施設外の集計対象（施設外就労・施設外支援）を返します。
