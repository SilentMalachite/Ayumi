...
出欠・実績記録票（月次・印刷対応）、モニタリング期限・受給者証期限のアラートを
一つの画面に集約し、支援業務に必要な情報の記録・確認・共有をまとめて行えます。

名前の「歩み」は設計思想を表しています。支援計画の段階遷移、短期目標の進捗、
日々の支援記録、出欠・実績記録——すべてを **追記専用のログ** として記録し、最新行から
「現在の状態」を導出します。過去の記録は上書きせず、訂正も新しい行として残すため、
福祉記録としての履歴が失われません。
