...
- **職員認証**（`phx.gen.auth`）: メールアドレス＋パスワード、またはマジックリンクでのログイン、
  アカウント設定（メール・パスワード変更）。
...
> **LAN の他の端末から接続するには:** 開発時の既定はループバック（`127.0.0.1`）にバインドされており、
> 他のマシンからはアクセスできません。`config/dev.exs`（本番は `config/runtime.exs`）の
> エンドポイント設定を `ip: {0, 0, 0, 0}` に変更してください。ポートは `PORT` 環境変数で変更できます。

## 初期化と職員アカウント

このアプリはオフライン運用でメール送信を行わないため、メールのマジックリンクによる通常の
アカウント登録は **本番では使えません**。職員アカウントは確定済み（メール確認不要）の状態で
直接作成します。
