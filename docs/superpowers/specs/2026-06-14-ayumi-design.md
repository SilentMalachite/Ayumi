...
- 実装状況メモ: 2026-06-18 時点で Step2 `goal_progress` は実装済み。
  実装では訂正専用の `correction_of_id` は持たず、`recorded_at` と `inserted_at` を保持し、
  最新判定は挿入順を表す `id` で行う。導出関数は独立 `Derive` モジュールではなく
  `Ayumi.Plans.current_goal_progress/1` などの context API に置いている。
...
実装済みの Step2 では `Ayumi.Plans.current_goal_progress/1`（DB 非依存・純粋）が正本:

- `current_goal_progress(rows)` = **`id` 昇順の最後の行**（最新勝ち）。
- `history(rows)` = 時系列順リスト（タイムライン表示用）。
- 空ログ → `nil`。

一覧・詳細は `list_goal_progress_for_goals/1` と `latest_goal_progress_by_goal/1` で
複数 goal 分をまとめて取得し、正本の純関数は不変のまま使う。
...
- **Step3（次）**: plan_phase_event + 現ステージ導出 + 締切クエリ + ダッシュボード + テスト。
