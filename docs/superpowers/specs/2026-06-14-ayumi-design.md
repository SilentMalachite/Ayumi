...
- 実装状況メモ: 2026-06-18 時点で Step2 `goal_progress` と Step3 `plan_phase_event` /
  モニタリング期限ダッシュボードは実装済み。実装では訂正専用の `correction_of_id` は持たず、
  `recorded_at` と `inserted_at` を保持し、最新判定は挿入順を表す `id` で行う。導出関数は
  独立 `Derive` モジュールではなく `Ayumi.Plans.current_goal_progress/1` /
  `Ayumi.Plans.current_plan_stage/1` などの context API に置いている。
...
2. **ダッシュボードは全員分の締切を表示**。ログイン中スタッフの担当利用者を先頭に寄せる
   ソートを添える（全員が見える点は不変）。
3. **目標の進捗ステージは提案の5段階で実装済み**。enum を1か所に集約し、施設の確認後に
   変更しやすい状態を保つ。
...
| recorded_at | utc_datetime | ✓ | 記録日時（サーバ側で付与） |
| note | text | | 所見 |
...
実装済みの Step2/Step3 では `Ayumi.Plans.current_goal_progress/1` と
`Ayumi.Plans.current_plan_stage/1`（DB 非依存・純粋）が正本:

- `current_goal_progress(rows)` / `current_plan_stage(rows)` = **`id` 昇順の最後の行**（最新勝ち）。
- `history(rows)` = 時系列順リスト（タイムライン表示用）。
- 空ログ → `nil`。

一覧・詳細は `list_goal_progress_for_goals/1` と `latest_goal_progress_by_goal/1` で
複数 goal 分をまとめて取得し、plan phase は `list_plan_phase_events/1` で履歴を取得する。
正本の純関数は不変のまま使う。
...
`Plans.list_monitoring_deadline_alerts(scope, today, near_days \\ 30)`:
...
- **Step2**: 計画詳細の各目標に「現在の進捗 + 進捗記録フォーム（ステージ選択 + 所見）」。送信で
  `goal_progress` を append、進捗履歴も表示。**実装済み**。
- **Step3**: 計画詳細にライフサイクル「現ステージ + ステージ記録」アクション（`plan_phase_event`
  を append）+ 履歴。**ダッシュボード（ログイン後トップ `/`）に締切（超過 + 30日以内、自分の担当先頭）**。
  **実装済み**。

修正フロー: 「訂正を記録」= 新規行を append。Step2 `goal_progress` では訂正先を指す
`correction_of_id` は持たない。
...
- 単体: 導出関数（最新勝ち / 空 → nil）、各 changeset（必須・enum・日付順）、
...
- **Step1（完了）**: service_user + support_plan + goal（スキーマ/移行/changeset/context/LiveView/テスト）。
- **Step2（完了）**: goal_progress + `Ayumi.Plans.current_goal_progress/1` + 進捗記録 UI + テスト。
- **Step3（完了）**: plan_phase_event + 現ステージ導出 + 締切クエリ + ダッシュボード + テスト。
...
- 目標進捗の5段階は現行仕様として実装済み。施設確認で語彙が変わる場合は enum と UI を更新する。
