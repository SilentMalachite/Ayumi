...
（2026-06-21 時点）。**1つずつ・TDDで・`mix review` がグリーンになってから次へ。**
...
## ✅ ステップ4 — `support_record`（支援記録）（完了済み）

目的: 利用者ごとの日々の支援内容を追記専用ログとして記録し、一覧・フィルタで確認。

- [x] `SupportRecord` スキーマ + マイグレーション（content, category, recorded_at, service_user_id, recorded_by_id）
- [x] `SupportRecordCategory` 列挙体（work / daily_living / health / interview / other）
- [x] context: `create_support_record/2`、`list_support_records/2`（利用者別・日付範囲フィルタ）
- [x] `SupportRecordLive.Index`（`/support_records`）: 一覧・新規作成・フィルタ
- [x] テスト（context / changeset / LiveView フロー）
...
目的: `ServiceUserLive.Show` を拡張し、1画面で利用者の全体像を把握できるようにする。
スキーマ変更なし。

- [x] `Plans.list_recent_support_records/2` — 利用者の支援記録を最新N件取得
- [x] `Plans.list_recent_goal_progress_for_user/2` — 利用者の全目標の進捗を最新N件取得
- [x] `Plans.list_recent_plan_phase_events_for_user/2` — 利用者の全計画のフェーズイベントを最新N件取得
- [x] 期限バッジ（受給者証期限・モニタリング期限の超過/近接表示）
- [x] 現行計画と目標の最新進捗ステータス
- [x] 進捗・フェーズ履歴（最近20件）
- [x] 支援記録（最近20件）+ 支援記録表へのリンク
- [x] 出欠・実績記録票へのリンク
- [x] テスト（context 単体テスト 9件 + LiveView テスト 7件）

---

## ✅ ステップ6 — 出欠・実績記録票（attendance_record）（完了済み）

目的: 1利用者・1か月の出欠／サービス提供を**追記専用ログ**として記録し、月次の
実績記録票を導出・印刷する。仕様変更時は **`Ayumi.Plans.ProvisionType` を一元的に
更新**して view・集計・テストを連動させる。

### 増分1 — データ基盤＋月次集計の純関数（完了済み）

- [x] `Ayumi.Plans.ProvisionType` 列挙体（`all/0` `label/1` `options/0` `billable/0` `offsite/0`）
- [x] マイグレーション `create_attendance_records`:
      `service_user_id` / `service_date` / `provision_type` / `pickup` / `dropoff` /
      `start_time` / `end_time` / `note` / `recorded_by_id` / `recorded_at` /
      `inserted_at`。`updated_at` なし。`(service_user_id, service_date)` 複合 index。
- [x] `Ayumi.Plans.AttendanceRecord` スキーマ + `changeset/2`（必須項目、
      `provision_type` の inclusion、`end_time > start_time` のバリデーション、
      FK 制約）+ `put_audit/3`（`recorded_by_id` / `recorded_at` をサーバ側で付与）。
- [x] `Ayumi.Plans.AttendanceSheet` 構造体（`lines` と `totals: %{billable_days,
      offsite_days, pickup_count, dropoff_count, absence_support_count}`）。
- [x] context:
      - [x] `change_attendance_record/2`、`create_attendance_record/2`（append-only）
      - [x] `list_attendance_records/3` + `month_bounds` ヘルパ
      - [x] `build_attendance_sheet/3` — ログを畳み込んで `AttendanceSheet` を返す純関数的導出
- [x] テスト（enum / changeset / context / 畳み込み）。

### 増分2 — 出欠入力 LiveView（月グリッド、完了済み）

- [x] `AyumiWeb.AttendanceLive.Index`（`/service_users/:service_user_id/attendance`、
      `:require_authenticated_user`、全認証スタッフ向け）。
- [x] 1利用者・1か月分を1日1行のグリッドで表示。年月の前後ナビゲーション。
- [x] `save_day` イベントで新規行を append（既存行の更新は行わない）。`recorded_by_id` と
      `recorded_at` はサーバ側で付与（フォーム値は無視）。
- [x] `end_time <= start_time` はエラーフラッシュで弾き、行は追記しない。
- [x] 同じ日の訂正は最新行が採用される（並びは表示時にログを畳み込んで決定）。
- [x] LiveView テスト（月レンダ／年境界ナビ／append／訂正最新採用／時刻不整合／監査フィールド無視）。

### 増分3 — 印刷ビュー（HTML、A4 最適化、完了済み）

- [x] `AyumiWeb.AttendanceLive.Sheet`（`/service_users/:service_user_id/attendance/sheet`、
      `:require_authenticated_user`）。
- [x] `@page { size: A4; margin: 12mm }` ＋ ツールバーは `print:hidden`。
- [x] 様式ヘッダ（事業所名・事業所番号）は `:ayumi, :facility` 設定から読み出し
      （未設定は空欄）。`config/config.exs` にコメントテンプレを追加。
- [x] 合計行は `AttendanceSheet.totals` を直接表示（画面で再計算しない）。
- [x] Index 画面から印刷ページへの導線リンク（年月引き継ぎ）。
- [x] 年月パラメータ解釈を `AyumiWeb.AttendanceLive.MonthParams` に抽出（Index / Sheet 共通化）。
- [x] `ProvisionType.offsite/0` を追加し、`totals_from/1` の施設外集計を一元化。
- [x] `priv/gettext/default.pot` / `ja/LC_MESSAGES/default.po` を再抽出。
- [x] LiveView テスト（アクセス／facility 設定／提供形態ラベル／合計一致／訂正最新採用）。

### 将来課題（実装しない／列挙のみ）

- [x] CSV 出力（月別実績の機械可読エクスポート）— `/exports` で週／月／年度／暦年を出力（`Ayumi.Exports`）。
  計画: `docs/superpowers/plans/2026-09-17-csv-export-import.md`
...
- [x] アプリ起動中のみ、LiveView hook から Web Notifications API で OS 通知を出す
      ナッジ。ブラウザごとの許可が前提なので保証層にはしない（画面内リストが保証）。
- [x] ロール分離（サビ管 vs 支援者）。`manager` / `supporter` の2ロール。
      データ管理（利用者・計画の作成・編集）はサービス管理責任者のみ。進捗記録・閲覧は全職員。
- [x] DB バックアップ（`Ayumi.Backups`）。SQLite の `VACUUM INTO` で稼働中 DB を
      整合的に書き出す。サービス管理責任者専用の LiveView `/admin/backup` と
      `mix ayumi.backup [dest]` から実行可能。保存先検証・稼働中 DB と同じ
      ディレクトリの拒否・同一秒衝突回避（`_1`, `_2` … サフィックス、上限 16）。

### ペンディング（フォロアップ候補）

- [ ] ビルド済みリリース（`bin/ayumi`）からコマンドラインでバックアップを実行する
      `Ayumi.Release.backup/1` ヘルパー。現状は Web 画面・Mix タスクのみ。
- [ ] `config.exs` の `:backup_dir` 既定値が `__DIR__` でビルド時パスに固定される
      問題。コンパイル済みリリースで意図しない値になる可能性。
